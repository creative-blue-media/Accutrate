...
module.exports = function (app, express) {
    ...
    InventoryApi.get("/inventorys/:limit", function (req, res) {
        console.log("Get Inventory Called?")
        console.log("Limit: ", req.params.limit);
        // console.log(req.body);

        Inventory.find().sort('-date').limit(parseInt(req.params.limit)).populate("_author").exec(function (err, inventorys) {
            if (err) console.log(err);

            res.status(200).send({
                success: true,
                message: "Inventorys retrieved",
                latest_inventorys: inventorys
            })
        })
    })
